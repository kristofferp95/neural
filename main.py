...
def get_args_parser():
    ...
    parser.add_argument(
        "--pretrained_weights", type=str, help="the path to pretrained weights file"
    )
    ...
    parser.add_argument(
        "--test_folder_seen", type=str, help="The seen test root folder"
    )

    parser.add_argument("--test_csv_unseen", type=str, help="The unseen test csv")
    parser.add_argument(
        "--test_folder_unseen", type=str, help="The unseen test root folder"
    )
...
def main(args):
    ...
    if args.mode == "train":
        dataset_train = phosc_dataset(
            args.train_csv, args.train_folder, transforms.ToTensor()
        )
        ...
        if args.valid_csv is not None or args.valid_folder is not None:
            ...
            dataset_valid = phosc_dataset(
                args.valid_csv, args.valid_folder, transforms.ToTensor()
            )
        ...
        dataset_test_seen = phosc_dataset(
            args.test_csv_seen, args.test_folder_seen, transforms.ToTensor()
        )
        ...
        dataset_test_unseen = phosc_dataset(
            args.test_csv_unseen, args.test_folder_unseen, transforms.ToTensor()
        )
    ...
    def training():
        ...
        for epoch in range(1, args.epochs + 1):
            mean_loss = train_one_epoch(
                model, criterion, data_loader_train, opt, device, epoch
            )
            valid_acc = -1
